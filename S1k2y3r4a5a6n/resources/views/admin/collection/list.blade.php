...
<div class="row">
    <div class="col-12">
        <div class="float-end"> @if(in_array('add',$privileges) || in_array('all',$privileges))  <a class="btn btn-s btn-lg" href="{{ route('admin.collection.create') }}">Create Collection</a> @endif </div>
...
var dataTable = $('#datatable').DataTable({
            ...
            "order": [[0, "desc"]],
            ajax: {
                url: '{!! route('admin.fetch.collection.data') !!}',
                data: function (d) {
    ...
    function delete_collection(id) {
        if (confirm('Are you sure! you want to delete?')) {
            $.post("{{ url('admin/collection') }}/"+id, {id: id, _method: 'DELETE', _token: '{{ csrf_token() }}'})
            .done(function (response) {
                if (response == 'ok')
                {
                    var table = $('#datatable').DataTable();
                    table.row('collection_dt_row_' + id).remove().draw(false);
                } else
                {
